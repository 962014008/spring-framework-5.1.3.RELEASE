...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource) throws BeanDefinitionStoreException {
		try {
			// 利用jdk的API，把inputSource封装成document文件对象
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 委托模式，把xml(document)的解析工作委托给BeanDefinitionDocumentReader实例完成
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		int countBefore = getRegistry().getBeanDefinitionCount();
		// 主要看这个方法，createReaderContext(resource)返回的xmlReaderContext，封装了xmlBeanDefinitionReader和ContextNamespaceHolder注册的spring.handlers解析类映射信息

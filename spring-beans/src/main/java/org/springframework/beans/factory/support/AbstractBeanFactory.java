...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
    ...
    protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
        ...
        // 解决单例无参构造器bean实例的循环依赖-关键点2
        // 当前bean如果作为属性依赖注入到另一个bean，自身又没有实例完成（isSingletonCurrentlyInCreation=true）的时候，
        // 会调用getSingleton(beanName, allowEarlyReference=true)，尝试从第三级缓存中获取未完全实例化的beanInstance，并升级到第二级缓存（提升效率）
        ...
        if (sharedInstance != null && args == null) {
            ...
            // 如果scope=prototype，校验是否出现循环依赖，如果有则直接报错
            ...
            try {
                ...
                if (mbd.isSingleton()) {
                    // 这里也是一个工厂模式的应用，下面的匿名对象工厂方法作为singletonFactory参数，
                    // 传入getSingleton(String beanName, ObjectFactory<?> singletonFactory)方法，
                    // 该方法会将已经完全实例化的bean实例添加到第一级缓存，并从第二级和第三级缓存中删除
                ...
                // prototype多例每次getBean都会创建新的bean，且默认具有lazy-init懒加载特性
                else if (mbd.isPrototype()) {
                    // It's a prototype -> create a new instance.
                    Object prototypeInstance;
                    try {
                        // 多例只有prototypesCurrentlyInCreation缓存，没有三级缓存
    ...
    protected Object getObjectForBeanInstance(Object beanInstance, String name, String beanName, @Nullable RootBeanDefinition mbd) {
        ...
        Object object = null;
        if (mbd == null) {
            // 从缓存factoryBeanObjectCache容器里面拿FactoryBean类型的实例
    ...
    protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
        ...
        if (!mbd.isPrototype() && requiresDestruction(bean, mbd)) {
            if (mbd.isSingleton()) {
                // 将收集到的destructionAwareBeanPostProcessor封装成disposableBeanAdapter，并注册到beanFactory
